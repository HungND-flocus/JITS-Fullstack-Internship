...
Hệ thống Mini-Wallet được chuyển đổi sang kiến trúc Config-Driven. Mô hình dữ liệu được chia làm 3 nhóm chính:

### 2.1. Nhóm Config (WHAT) - Admin khai báo
Đây là các bảng lưu trữ cấu hình nghiệp vụ. Dữ liệu trong này do Admin tạo ra ở thời điểm Design-time. Engine sẽ đọc các bảng này để biết phải làm gì.
- **`Service`**: Định nghĩa một nghiệp vụ. Bao gồm `code`, `name`, cấu hình xác thực (`auth`), cấu hình phí (`fee`), và đặc biệt là danh sách `fieldBuilder` (luật dựng các biến dùng chung từ input thô).
- **`TransField`**: Định nghĩa và kiểm tra định dạng của từng biến (VD: kiểu String, độ dài tối đa, bắt buộc nhập không).
- **`TransValidation`**: Định nghĩa các luật kiểm tra nghiệp vụ trước khi cho phép giao dịch (VD: Số dư phải lớn hơn số tiền chuyển + phí).
...
Nhóm này lưu vết lại toàn bộ quá trình dòng tiền chạy.
- **`TransactionTrail`**: "Hồ sơ bệnh án" của một giao dịch, sống xuyên suốt qua 3 bước (Request -> Confirm -> Verify). Dùng `transRefId` để làm mã tra cứu.
- **`Transaction`**: "Biên lai" giao dịch. Chỉ được sinh ra ở bước cuối cùng (Verify) nếu tiền đã thực sự được chuyển thành công.
- **`Pocket` (Ví)**: Lưu trữ số dư (`balance`) của các chủ thể. Được bảo vệ bằng mã băm `checksum` chống sửa tay trực tiếp trong Database.
- **`PocketEntry`**: Dấu vết chi tiết của từng dòng tiền (Bút toán). Mỗi `glStep` chạy thành công sẽ sinh ra một `PocketEntry`.
...
- **`Officer`**: Quản trị viên hệ thống. Người chuyên đi tạo cấu hình.
- **`Biller`**: Nhà cung cấp hoá đơn (Điện, Nước). Có URL để tra cứu và thanh toán.
